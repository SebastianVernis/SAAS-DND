...
export async function getProjects(req, res, next) {
  try {
    ...
    // Use cache wrapper - cache for 5 minutes (300s)
...
export async function createProject(req, res, next) {
  try {
    ...
    // Invalidate organization projects cache
...
export async function getProject(req, res, next) {
  try {
    ...
    // Cache key for this specific project
    const cacheKey = projectCacheKey(projectId, 'full');

    // Use cache wrapper - cache for 10 minutes (600s)
...
export async function updateProject(req, res, next) {
  try {
    ...
    // Invalidate project and organization caches
...
export async function duplicateProject(req, res, next) {
  try {
    ...
    // Invalidate organization cache (new project added)
    await invalidateOrgCache(organizationId);
