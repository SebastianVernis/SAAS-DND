 ...
 * @param {!CodeMirror} cm
 * @param {!number} line
 * @return {Array} (Cached) array of tokens
 */
...
export function getTokenAt(cm, pos, precise) {
    ...
    // Binary search for token (lodash sortedIndex replacement)
    let tokenIndex = 0;
    for (let i = 0; i < cachedTokens.length; i++) {
        if (cachedTokens[i].end >= pos.ch) {
            tokenIndex = i;
            break;
        }
    }
    ...
    return token || cm.getTokenAt(pos, precise);  // fall back to CMs getTokenAt
}
...
export function offsetInToken(ctx) {
    ...
    if (offset < 0) {
        console.log("TokenUtils: offsetInToken - Invalid context: pos not in the current token!");
    }
    return offset;
}
...
export function getModeAt(cm, pos, precise) {
    ...
    if (modeData.innerMode) {
        // For CodeMirror 6, we need to handle this differently
        // This is a simplified version that works with basic modes
        modeData = modeData.innerMode ? modeData.innerMode(getTokenAt(cm, pos, precise).state) : modeData;
    }
